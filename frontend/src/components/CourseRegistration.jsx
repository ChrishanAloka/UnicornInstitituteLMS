...
const CourseRegistration = () => {
    ...
    const [loadingExtraSessions, setLoadingExtraSessions] = useState(false);
    const [absentDays, setAbsentDays] = useState([]);
    const [loadingAbsentDays, setLoadingAbsentDays] = useState(false);
    ...
    const fetchExtraSessions = async () => {
        try {
            ...
            console.error("Failed to fetch extra sessions:", err);
        } finally {
            setLoadingExtraSessions(false);
        }
    };
    ...
    console.log("enriched ", enrichedCourses);
    ...
    const organizeTimetable = () => {
        ...
        // Create maps for rescheduled, extra, and absent sessions
        const rescheduleMap = {};
        ...
        // Create map for absent days (courseId + date)
        const absentDayMap = {};
        absentDays.forEach(absent => {
            const dateStr = new Date(absent.absentDate).toISOString().split('T')[0];
            const key = `${absent.course._id}_${dateStr}`;
            absentDayMap[key] = absent;
        });

        // Create map for extra sessions by date
        const extraSessionMap = {};
        extraSessions.forEach(extra => {
            const dateStr = new Date(extra.extraDate).toISOString().split('T')[0];
            if (!extraSessionMap[dateStr]) {
                extraSessionMap[dateStr] = [];
            }
            extraSessionMap[dateStr].push(extra);
        });
        ...
        enrichedCourses.forEach(course => {
            ...
            weekDates.forEach(({ day, date }) => {
                ...
                if (isRegularDay) {
                    // Check if this date is marked as absent
                    const absentKey = `${course._id}_${currentDateStr}`;
                    const isAbsent = !!absentDayMap[absentKey];

                    if (isAbsent) {
                        // Skip this session - it's marked as absent
                        return;
                    }
                    ...
                    const isRescheduled = courseReschedules.some(rs => {
                        ...
                        return rsOriginalDate === currentDateStr;
                    });
        ...
        // Add extra sessions to the timetable
        weekDates.forEach(({ day, date }) => {
            const dayKey = day.toLowerCase();
            const dateStr = date.toISOString().split('T')[0];

            if (extraSessionMap[dateStr]) {
                extraSessionMap[dateStr].forEach(extra => {
                    const course = enrichedCourses.find(c => c._id === extra.course._id);
                    if (course) {
                        timetable[dayKey].push({
                            ...course,
                            isRescheduled: false,
                            ...
                            extraStartTime: extra.startTime,
                            extraEndTime: extra.endTime,
        ...
        Object.keys(timetable).forEach(day => {
            timetable[day].sort((a, b) => {
                // Extra sessions should appear first
                if (a.isExtra && !b.isExtra) return -1;
                if (!a.isExtra && b.isExtra) return 1;

                // Then sort by actual time
                const aTime = a.isExtra ? a.extraStartTime : a.timeFrom;
                const bTime = b.isExtra ? b.extraStartTime : b.timeFrom;
    ...
    // Updated to handle extra sessions
    const getDisplayTime = (session) => {
        if (session.isExtra) {
            return `${session.extraStartTime} - ${session.extraEndTime}`;
        }
        return `${session.timeFrom} - ${session.timeTo}`;
    };
...
`;
    ...
    const handleExportExcel = () => {
        try {
            ...
            // Add extra sessions summary
            excelData.push([]);
            excelData.push(['EXTRA SESSIONS THIS WEEK']);
            excelData.push(['Course', 'Date', 'Time', 'Reason']);
            extraSessions
                .filter(extra => {
                    const extraDate = new Date(extra.extraDate);
                    const weekStart = getWeekDates()[0].date;
                    const weekEnd = getWeekDates()[6].date;
                    return extraDate >= weekStart && extraDate <= weekEnd;
                })
                .forEach(extra => {
                    excelData.push([
                        extra.course?.courseName || 'Unknown',
                        new Date(extra.extraDate).toLocaleDateString('en-GB'),
                        `${extra.startTime} - ${extra.endTime}`,
                        extra.reason || ''
                    ]);
                });
            // Add absent days summary
            excelData.push([]);
            excelData.push(['ABSENT DAYS THIS WEEK (CANCELLED SESSIONS)']);
            excelData.push(['Course', 'Absent Date', 'Regular Time', 'Reason']);
            absentDays
                .filter(absent => {
                    const absentDate = new Date(absent.absentDate);
                    const weekStart = getWeekDates()[0].date;
                    const weekEnd = getWeekDates()[6].date;
                    return absentDate >= weekStart && absentDate <= weekEnd;
                })
                .forEach(absent => {
                    const course = enrichedCourses.find(c => c._id === absent.course._id);
                    excelData.push([
                        absent.course?.courseName || 'Unknown',
                        new Date(absent.absentDate).toLocaleDateString('en-GB'),
                        course ? `${course.timeFrom} - ${course.timeTo}` : 'N/A',
                        absent.reason || ''
                    ]);
                });
            // Create worksheet
    ...
    const getCourseTime = (courseId) => {
        const course = enrichedCourses.find(c => c._id === courseId);
        if (!course) return { startTime: '', endTime: '' };
        return { startTime: course.timeFrom, endTime: course.timeTo };
    };
                    ...
                    <div className="modal-dialog modal-xl">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ...
                                {/* Loading Indicator */}
                                {(loadingReschedules || loadingExtraSessions || loadingAbsentDays) && (
                                ...
                                <div id="timetable-print">
                                                                        ...
                                                                        {overlappingCourses.map((course, idx) => {
                                                                            ...
                                                                            return (
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    {course.isExtra && (
                                                                                        <span className="ms-1 badge bg-light text-success" style={{ fontSize: '0.6rem' }}>
                                                                                            Extra
                                                                                        </span>
                                                                                    )}
                                                                                    {course.isRescheduled && (
                                    ...
                                    <div className="mt-4 p-3 rounded">
                                        ...
                                        <div className="d-flex flex-wrap gap-4">
                                            ...
                                            <div className="d-flex align-items-center">
                                                <div className="bg-success text-white p-2 rounded me-2" style={{ width: '20px', height: '20px' }}></div>
                                            ...
                                            <div className="d-flex align-items-center">
                                                ...
                                                <span>Extra Session Indicator</span>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <table className="table table-sm table-bordered">
                                                    ...
                                                    <tbody>
                                                        {extraSessions
                                                            ...
                                                            .map((extra, idx) => (
                                                                <tr key={idx}>
                                                                    <td className="fw-semibold">{extra.course?.courseName || 'Unknown'}</td>
                                                                    <td className="text-success fw-bold">
                                                                        {new Date(extra.extraDate).toLocaleDateString('en-GB')}
                                                                    </td>
                                                                    <td>
                                    ...
                                    {absentDays.length > 0 && (
                                        <div className="mt-4 p-3 bg-danger bg-opacity-10 rounded border border-danger">
                                            <h6 className="mb-3">
                                                ...
                                                Absent Days This Week (Sessions Cancelled)
                                            </h6>
                                            <div className="table-responsive">
                                                <table className="table table-sm table-bordered">
                                                    <thead className="table-light">
                                                    ...
                                                    <tbody>
                                                        {absentDays
                                                            ...
                                                            .map((absent, idx) => {
                                                                ...
                                                                return (
                                                                    <tr key={idx}>
                                                                        <td className="fw-semibold">{absent.course?.courseName || 'Unknown'}</td>
                        ...
                        <div className="modal-content">
                            <div className={`modal-header ${
                                ...
                                <h5 className="modal-title">
                                    ...
                                    {selectedCourseDetails.isExtra && '[Extra] '}
                                    {selectedCourseDetails.isRescheduled && '[Rescheduled] '}
                                    {selectedCourseDetails.courseName}
                                </h5>
                                <button
                                    className={`btn-close ${selectedCourseDetails.isExtra || selectedCourseDetails.isRescheduled ? '' : 'btn-close-white'}`}
                                    onClick={() => {
                                    ...
                                    <div className="col-md-7">
                                        <div className="mb-4">
                                            ...
                                            <div className="d-flex mb-3">
                                                <span className="text-muted" style={{ minWidth: '100px' }}>Session Type:</span>
                                                <span className="fw-semibold">
                                                    ...
                                                    {!selectedCourseDetails.isExtra && !selectedCourseDetails.isRescheduled && (
                                            ...
                                            <div className="d-flex mb-3">
                                                ...
                                                <span className="fw-semibold text-primary">
                                                    ...
                                                    {selectedCourseDetails.isExtra
                                                        ? `${selectedCourseDetails.extraStartTime} - ${selectedCourseDetails.extraEndTime}`
                                        ...
                                        {selectedCourseDetails.isExtra && (
                                            <div className="alert alert-success">
                                                ...
                                                <div className="d-flex mb-2">
                                                    <span className="text-muted" style={{ minWidth: '120px' }}>Session Date:</span>
                                                    <span className="fw-bold text-success">
                                                        <i className="bi bi-calendar2-plus me-1"></i>
                                                        {new Date(selectedCourseDetails.extraDate).toLocaleDateString('en-GB', {
                                                            weekday: 'long',
                                                            day: 'numeric',
                                                            month: 'long',
                                                            year: 'numeric'
                                                ...
                                                <div className="d-flex mb-2">
                                                    <span className="text-muted" style={{ minWidth: '120px' }}>Session Time:</span>
                                                    <span className="fw-bold">
                                                        <i className="bi bi-clock me-1"></i>
                                                        {selectedCourseDetails.extraStartTime} - {selectedCourseDetails.extraEndTime}
                                                    </span>
                                                </div>
                                                {selectedCourseDetails.extraReason && (
                                                    <div className="d-flex">
                                                        <span className="text-muted" style={{ minWidth: '120px' }}>Reason:</span>
                                                        <span className="fw-semibold">{selectedCourseDetails.extraReason}</span>
                                                    </div>
                                                )}
                                            </div>
                                        )}
